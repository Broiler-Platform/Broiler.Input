...
public sealed record LinuxEventDeviceAccessStatus(string InputDirectory, bool DirectoryExists, int EventDeviceCount,
    int ReadableEventDeviceCount, string Diagnostic);
...
public static class LinuxEventDeviceAccessProbe
{
    ...
    public static IReadOnlyList<string> GetEventDevicePaths(string inputDirectory = DefaultInputDirectory)
    {
        ...
        try
        {
            return [.. Directory.EnumerateFiles(inputDirectory, "event*").OrderBy(static path => path, StringComparer.Ordinal)];
        }
    ...
    public static LinuxEventDeviceAccessStatus CheckEventDeviceAccess(string inputDirectory = DefaultInputDirectory)
    {
        if (string.IsNullOrWhiteSpace(inputDirectory))
        {
            return new LinuxEventDeviceAccessStatus(inputDirectory, DirectoryExists: false, EventDeviceCount: 0, ReadableEventDeviceCount: 0,
        ...
        return new LinuxEventDeviceAccessStatus(inputDirectory, DirectoryExists: true, eventDevices.Count, readable, diagnostic);
    }
