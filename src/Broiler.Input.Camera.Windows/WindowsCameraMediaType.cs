...
internal static class WindowsCameraMediaType
{
    public static CameraFormat ReadFormat(IMFMediaType mediaType)
    {
        ...
        int result = mediaType.GetGUID(ref majorTypeKey, out Guid majorType);
        ...
        WindowsCameraFaults.ThrowIfFailed(result, "Media Foundation camera media subtype lookup failed.");
        ...
        return new CameraFormat(checked((int)width), checked((int)height), checked((int)frameRateNumerator),
            checked((int)frameRateDenominator), ToPixelFormat(subtype));
    }
